...
a simplistic scoring system that used weighted values for the board positions.  Because of this,
...
avoided.  Other benefits are the ability to review and amend the design before it’s executed, and having the documentation
...
another design.  So I ask Rust to implement min-max A/B pruning [^3], the algorithm I did in C++ a long time ago in a galaxy far, far away.  Reviewing the documentation is important.  With the min-max implementation the generated code examples in the document used 1000 for a winning state and -1000 for a losing state.  This was a bad design decision.  Any algorithm that evaluated a board outside of those bounds would erratically choose suboptional moves because it would treat them as winning or losing.  I instructed Cursor to use `f64::INFINITY` for winning, and `f64::NEG_INFINITY` for losing and it updated the document accordingly.
